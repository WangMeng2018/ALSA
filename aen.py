...
from keras.layers import Bidirectional, Dense, Dot, Lambda, LeakyReLU, MaxPooling1D
...
def my_model(opt, word_index, embedding_matrix):
    ...
    # Concatenate
    asen_pool = MaxPooling1D(opt['max_sentence_length'])(asen)
    aasp_pool = MaxPooling1D(N_ASPECT)(aasp)
    X_pool = MaxPooling1D(N_ASPECT)(X)
    print(asen_pool)
    print(aasp_pool)
    print(X_pool)
    X = Concatenate(axis = 1)([asen_pool,X_pool,aasp_pool])
    X = Dropout(opt['drop_rate'], name='Dropout')(X)
    # X = [batch_size, 3 * lstm_hidden_size]
