...
from keras.layers import Input, Embedding, LSTM, Softmax, Flatten, Dropout, Concatenate
...
def my_model(opt, word_index, embedding_matrix):
    ...
    sen = Input(shape=(opt['max_sentence_length'],), name='Sentence', dtype='float32')
    # asp = [1, N_ASPECT]
    asp = Lambda(
    ...
    # aasp and asen = [batch, 2*lstm_hidden_size]
    sen_avg = Lambda(lambda x: K.mean(x,axis=1))(asen)
    asp_avg = Lambda(lambda x: K.mean(x,axis=1))(aasp)

    # Interactive Attention Module
    # att_sen = [batch_size, max_sentence_len]
    att_sen = Dot(-1, name='Project_att_sen')([asen, asp_avg])
    att_sen = Dense(opt['max_sentence_length'], activation = 'tanh',name = 'weight_att_sen')(att_sen)
    att_sen = Softmax(1,name='att_sen_weight')(att_sen)
    att_context = Dot(1,name = 'att_context')([att_sen,asen])
    # att_asp = [batch_size, N_ASPECT]
    att_asp = Dot(-1,name='Project_att_asp')([aasp,sen_avg])
    att_asp = Dense(N_ASPECT, activation = 'tanh',name = 'weight_att_asp')(att_asp)
    att_asp = Softmax(1,name='att_asp_weight')(att_asp)
    att_aspect = Dot(1,name = 'att_aspect')([att_asp,aasp])
    # concatenate
    X = Concatenate(axis=1)([att_context,att_aspect])
